...
double Energy_vis(double L, double momentum, double Edep){
    ...
    // double xi = K * ZA * pow((1/beta),2);		// Xi variable 

    double dEdx = K * rho * ZA * pow((1/beta),2);		// Xi variable 
	// double DeltaAv = dEdx * (log((2*me*(pow(bg,2)))*WM/(pow(I,2)))-(2 * pow(beta,2))-(d));// Mean energy loss (Bethe-Bloch)

    double k_birks = 0.6; // Constante de Birks (cm MeV^-1)
    double E_vis = Edep / (1 + k_birks * dEdx);
...
void Muon_Gen_1(){
    ...
    int number_thet = 100000;

    // ============ Datos para el ICN ================== //
    double Radio = 8; //cm
    double half_plane_size = 0.75; // cm 

    double sizex_pixels = 250; // px
    double sizey_pixels = 529; // px
    ...
    TFile *file = TFile::Open("Sim_ab_initio_NMUONS_100000_PLANES_1.5_RADIO_8_CCDSIZE_250X529_SIGMA_1.0_SD_Log_BIRKS_0.1_C_.root", "recreate");
    // TFile *file = TFile::Open("Sim_ab_initio__CONNIE_NMUONS_1000000_PLANES_1.7_RADIO_7_CCDSIZE_1022X420_SIGMA_1.0_SD_Log_BIRKS_0.6_C_.root", "recreate");
    ...
    for (int i = 0; i<number_thet;i++){
        ...
        if ((0 < Delta_L) & (Delta_L < max_long)){
            ...
            // Cálculo de la energía visible con Birks //
            Evis = Energy_vis(Delta_L, momentum, Rand_edep);
