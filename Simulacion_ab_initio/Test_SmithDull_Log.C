...
void Test_SmithDull_Log(){
    ...
    for (int i = 0; i <100000;i++){
        double rand_Val = f->GetRandom();
        // hist->Fill(pow(10, rand_Val));
        hist->Fill(rand_Val);
    }
	...
	for (int i = 0;i < 100000;i++){
        ...
        hist2->Fill(pow(10,val));
    }

    // hist2->Scale(0.000000000002);

    // for (int i = 1; i<NB; i++){
    //     double cnt = hist2->GetBinContent(i);
    //     double w = hist2->GetBinWidth(i);
    //     double val = cnt/w;
    //     hist2->SetBinContent(i, val);
    // }

    // ================================================== //
    double cont_f = f->Integral(1, 5);
    double cont_hist = hist->Integral();

    hist->Scale(0.000000000004);
