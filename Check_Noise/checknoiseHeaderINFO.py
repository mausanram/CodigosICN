...
def parser(args):
    ...
    parser = argparse.ArgumentParser(prog='Check Noise',description='Displays a dataframe of image ID, Standard Deviation and Noise for each extension and can display extra information for each image (ex. Temperature, Voltage File, Exposure,...), and plots the histogram for each image.')
    ...
    parser.add_argument('--Noise',type = str, nargs= '+', help = "Displays a DataFrame sorted by noise column (Noise ext1), the histogram and the noise plot by extension")
