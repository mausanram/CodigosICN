...
def hist_and_DataFr(files,optionFlag = 0, graphicFlag = 0):
    ...
    active_mask = np.s_[:, 9:550]		#   9 <= x < 538
    overscan_mask = np.s_[:, 550:]		# 538 <= x
    mask=np.s_[:, :]			# Area where variable will be computed
    expgain = [210, 210, 193, 190]
    ...
    for image in files:
        ...
        for i in np.arange(0, len(hdul)):
        # for i in (0, 1, 3):

            # Load data and header
            data=hdul[i].data
            # data = hdul[i].data[:, 600:]
            ...
            if data is not None:				# Check if data is not empty
                ...
                hist, bin_edges = np.histogram(data[mask].flatten(), bins=100000)
                ...
                bin_heights, bin_borders, _ = axs_all[figctr].hist(data[mask].flatten(), range=[-300, 400], bins=1000, histtype='step', label=hlabel)
                # bin_heights, bin_borders, _ = axs_all[figctr].hist(data.flatten(), range=[-300, 300], bins=Bins, histtype='step', label=hlabel)
                ...
                print('Realizando el ajuste gaussiano de extensión: ', i)
...
def Noise(files, parplotFlag = 1):
    ...
    # expgain = [213, 2010, 202.97945260519987, 193.2145155088731]
