...
def main(argObj):
    ...
    for i in range(0, len(hdul)):
        ...
        offset = bins_edges[np.argmax(hist)]
        data = data - offset
        dataCal = (electron_to_keV*data)/expgain[i]
    ...
    ## Figura Completa
    fig_all  = plt.imshow(todo, vmin = dataCal.min(), vmax = dataCal.max(), origin='lower')#, cmap = color)
